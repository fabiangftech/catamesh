...
class CheckIfExistResourceDefinitionVersionHandlerTest {

    @Test
    void doHandleAddsNoopDefinitionPlanAndResolvesResourceId() {
        Query<GetResourceDefinitionDTO, Optional<String>> optionalDefinitionQuery = dto -> Optional.of("definition-1");
        Query<GetResourceDTO, Optional<Resource>> optionalResourceQuery = dto -> Optional.of(existingResource("resource-1"));
        CheckIfExistResourceDefinitionVersionHandler handler = new CheckIfExistResourceDefinitionVersionHandler(
                optionalDefinitionQuery,
                optionalResourceQuery
        );

        ApplyDataProductContext context = ApplyDataProductContext.create("yaml");
        context.setDataProduct(dataProduct(resource("orders", "0.0.1")));
        ...
        Assertions.assertEquals(PlanAction.NOOP, planResource.getAction());
        Assertions.assertEquals(1, context.getPlan().getSummary().getNoop());
    }
    ...
    private Resource existingResource(String resourceId) {
        Resource resource = resource("orders", "0.0.1");
